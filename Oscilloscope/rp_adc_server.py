...
def adc_dma_init():
    ...
    pin.GPIO_CTRL_REG = devs.GPIO_FUNC_NULL # type:ignore 
    pad.PAD_REG = 0 # type:ignore 

    dma.CHAN_ABORT = 0xffff # type:ignore 
    dma_chan.CTRL_TRIG_REG = 0 # type:ignore 

    adc.CS_REG = adc.FCS_REG = 0 # type:ignore 
    adc.CS.EN = 1
    adc.FCS.EN = adc.FCS.DREQ_EN = 1
    adc.FCS.THRESH = adc.FCS.OVER = adc.FCS.UNDER = 1
    adc.CS.AINSEL = ADC_CHAN

    dma_chan.READ_ADDR_REG = devs.ADC_FIFO_ADDR # type:ignore 
    dma_chan.CTRL_TRIG_REG = 0 # type:ignore 
...
def adc_capture():
    ...
    adc.DIV_REG = (48000000 // rate - 1) << 8 # type:ignore 
    dma_chan.WRITE_ADDR_REG = uctypes.addressof(adc_buff)# type:ignore 
    dma_chan.TRANS_COUNT_REG = nsamp # type:ignore 
